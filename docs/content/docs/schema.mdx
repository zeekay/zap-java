...
ZAP schemas define your data structures in a language-independent format. The `capnp` compiler generates type-safe Java code from these schemas.
...
| ZAP | Java Type | Size |
|-------------|-----------|------|
| `Void` | `org.capnproto.Void` | 0 bits |
| `Bool` | `boolean` | 1 bit |
| `Int8` | `byte` | 8 bits |
| `Int16` | `short` | 16 bits |
| `Int32` | `int` | 32 bits |
...
// or
result.setPending(org.capnproto.Void.VOID);
...
```bash
# Single file
capnp compile -ojava:src/main/java schema.capnp

# Multiple files
capnp compile -ojava:src/main/java *.capnp

# With include path
capnp compile -I/path/to/includes -ojava:out schema.capnp
```
